...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-534_passing-427819?style=for-the-badge" alt="534 tests passing"/>
</p>
...
| [guess.miru](examples/guess.miru) | A guessing game: `random_int`, `seed`, and `input` in a loop |
...
| ----- | ----- | ----- | -------------- |
| **Lexer** | token.rs, lexer.rs | 1756 | Source text to tokens, with line, column, and span tracking |
| **Parser** | ast.rs, parser.rs | 2410 | Recursive descent plus a Pratt expression parser |
| **Runtime model** | value.rs, ops.rs, builtins.rs, random.rs | 4919 | Values, operator and indexing rules, the builtin library |
| **Bytecode engine** | chunk.rs, globals.rs, compiler.rs, vm.rs | 4911 | Compiles the AST to bytecode, runs it on a stack VM, loads modules, and catches errors |
| **Diagnostics** | suggest.rs | 220 | Chooses the name an error offers back when a program misspells one |
| **Formatter** | formatter.rs | 930 | Reprints a program in canonical form (`miru fmt`) |
| **Migration** | migrate.rs | 639 | Renames what version 2.0 reserves, and reports what no tool should rewrite (`miru migrate`) |
...
| **Total** | **20 files** | **18647** | Written from scratch in Rust |

The playground is a separate crate: 800 lines of Rust binding the language to
